...
% file (in settings) operating on blocks of 32 seconds of data.
%
...
% the 1msec accumulations.  After processing all satellites in the 32 sec
...
if (fid > 0)
    ...
    fseek(fid, dataAdaptCoeff*settings.skipNumberOfSamples*settings.Datalength, 'bof');
    ...
    if ((settings.skipAcquisition == 0))
        ...
        data = fread(fid, ...
            dataAdaptCoeff*50*samplesPerCode, settings.dataType)';
        ...
        if settings.acquisitionType == 0 
            acqResults = acquisitionMode1(data, settings);
        else
            acqResults = acquisitionMode2(data, settings);
        end
